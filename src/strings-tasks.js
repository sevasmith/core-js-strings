...
function removeFirstOccurrences(str, value) {
  ...
  if (indexOfFirst !== -1) {
    const newString =
      ...
      str.substring(indexOfFirst + value.length);
    return newString;
  }
  return str;
}
...
function removeLastOccurrences(str, value) {
  ...
  if (indexOfLast !== -1) {
    const newString =
      str.substring(0, indexOfLast) + str.substring(indexOfLast + value.length);
    return newString;
  }
  return str;
}
...
function getStringFromTemplate(firstName, lastName) {
  const newString = `Hello, ${firstName} ${lastName}!`;
  return newString;
}
...
function extractNameFromTemplate(value) {
  ...
  const name = parts[1].substring(0, parts[1].length - 1);
  return name;
}
